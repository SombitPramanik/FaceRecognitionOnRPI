...
def FaceRecognition(frame, face_locations):
    ...
    # Use multiprocessing to recognize faces
    with Pool(cpu_count()-11) as pool:
        names_with_accuracy = pool.map(FRDependency, [(encoding, knn_clf) for encoding in face_encodings])
